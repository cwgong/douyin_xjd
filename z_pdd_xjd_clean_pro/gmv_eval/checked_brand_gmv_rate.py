...
class GMVEvalFunc(object):
    ...
    def getting_checked_product_dict(self):
        b_id_dict = {}
        with open("../top_brand.txt", "r", encoding="utf-8") as f1:
            for line in f1:
                line = line.strip()
                if line == "": continue
    ...
    def loading_all_product_info(self):
        ...
        with open(self.all_product_gmv_file) as f1:
            for line in f1:
                line = line.strip()
                if line == "": continue
                lst1 = line.split('\t')
                ...
                p_id, b_id, b_name, p_name, cat2_id, cat2_name, gmv = lst1
                if p_id == "": continue
                k1 = "%s|%s" % (b_id, b_name)
                if k1 not in self.checked_brand_dict: continue
                try:
    ...
    def xiaowu_brand_top10_product_stat(self):
        ...
        with open("gmv_output/品牌-高GMV-Top10商品.txt", "w") as f1:
            f1.write("\n".join(r_lst))
            f1.flush()
    ...
    def xiaowu_brand_cat1_gmv(self):
        ...
        with open("gmv_output/品牌-一级类目-GMV-统计.txt", "w") as f5:
            f5.write("\n".join(r_lst))
            f5.flush()
...
if __name__ == "__main__":
    obj = GMVEvalFunc("z_gmv.txt", "../top_brand.txt")
    #obj.brand_stat_info()
    obj.stat_go()
    ...
    for d in data:
        ...
        if p_id == None or b_id == None or b_name == None:
            ...
            continue
        b_id = str(b_id).strip()
        if b_id not in obj.checked_brand_dict: continue
        p_id = str(p_id).strip()
        b_name = b_name.strip()
        r_lst.append("%s\t%s\t" % d)

    with open("chked_info.txt", 'w') as f9:
        f9.write("\n".join(r_lst))
        f9.flush()
    '''